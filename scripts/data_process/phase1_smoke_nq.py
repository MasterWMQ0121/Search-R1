...
def make_prompt(question: str) -> str:
    return (
        ...
        "PHASE-1 FIXTURE MATCHING RULE: Find the single retrieved document whose Question: text "
        "exactly matches the original user question below. Ignore every other retrieved document, "
        "even if it also contains \"Accepted answer evidence:\". From ONLY the matching document, "
        "copy the text immediately following \"Accepted answer evidence:\". Once that matching "
        "answer-bearing document is found, you MUST NOT search again. Your next response MUST be "
        "exactly one final-answer block and nothing else.\n\n"
        "UNRELATED CONCRETE FORMAT EXAMPLE:\n"
        ...
        "<answer>blue</answer>\n"
        "No prose is allowed outside the opening and closing answer tags shown above. Stop "
        "immediately after </answer>. This matching rule applies only to this deliberately "
        "answer-leaky Phase-1 fixture.\n"
        "If no retrieved document exactly matches the original question and supplies accepted "
        "answer evidence, use <think>...</think> followed by exactly "
